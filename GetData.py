...
def getData(driver:webdriver, dataStorage: DataStorage):
    ...
    while not dataStorage.ifBet :
        ...
        for i in range(1,11) :
            ...
            logging.info("第"+str(i)+"名前"+str(observeRange)+"冷門號碼：" + str(resultList))
        ...
        logging.info(dataStorage.period + "期各名次連續開中冷門獎號次數：\n"+ countString)
        print(dataStorage.period + "期各名次連續開中冷門獎號次數：\n"+ countString)

        observeTime = dataStorage.observeTime
        for i in range(10):
            if dataStorage.countList[i] == observeTime:
                print("第"+str(i+1)+"名已開冷門達"+ str(observeTime) +"次")
                logging.info("第"+str(i+1)+"名已開冷門達" + str(observeTime) + "次")
                dataStorage.ifBet = True
                dataStorage.indexOfBet.append(i)
...
def getBetSymbol(driver:webdriver, dataStorage: DataStorage):
    ...
    logging.info(str(targetKey)+"期最終下注號碼：" + str(resultList))
...
def getPriceNumber(driver:webdriver, dataStorage: DataStorage):
    ...
    while True :
        ...
        logging.info("本輪開獎號碼："+str(compareList))
        logging.info("本輪中獎號碼："+str( list(set(compareList).intersection(dataStorage.BetSymbolList) ) ) )
        ...
        logging.info("本輪下注中獎"+ str(priceCount * betAmount * 9.9) + "元")
        logging.info("目前餘額 ： " + str(dataStorage.betMoneyTotal) + "元")
        return
